...
class AbstractBuildSubordinate(unittest.TestCase):

    class ConcreteBuildSubordinate(buildsubordinate.AbstractBuildSubordinate):
        pass

    class ConcreteBuildSubordinateTwo(buildsubordinate.AbstractBuildSubordinate):
        pass

    def test_constructor_minimal(self):
        bs = self.ConcreteBuildSubordinate('bot', 'pass')
        self.assertEqual(bs.subordinatename, 'bot')
        self.assertEqual(bs.password, 'pass')
        self.assertEqual(bs.max_builds, None)
        self.assertEqual(bs.notify_on_missing, [])
        self.assertEqual(bs.missing_timeout, 3600)
        self.assertEqual(bs.properties.getProperty('subordinatename'), 'bot')
    ...
    def test_constructor_notify_on_missing_not_string(self):
        self.assertRaises(AssertionError, lambda :
            self.ConcreteBuildSubordinate('bot', 'pass',
                    notify_on_missing=['a@b.com', 13]))

    def test_identity(self):
        bot = self.ConcreteBuildSubordinate('bot', 'pass').identity()
        bot2 = self.ConcreteBuildSubordinate('bot', 'pass').identity()
        boot = self.ConcreteBuildSubordinate('boot', 'pass').identity()
        two = self.ConcreteBuildSubordinateTwo('bot', 'pass').identity()
    ...
    def test_update(self):
        old = self.ConcreteBuildSubordinate('bot', 'pass',
                max_builds=2,
                notify_on_missing=['me@me.com'],
                missing_timeout=120,
                properties={'a':'b'},
                keepalive_interval=60)
        new = self.ConcreteBuildSubordinate('bot', 'pass',
                max_builds=3,
    ...
    def test_setBotmain(self):
        bs = self.ConcreteBuildSubordinate('bot', 'pass')
        bm = mock.Mock(name='botmain')
        ...
        # re-setting the botmain should fail
        self.assertRaises(AssertionError, lambda :
                bs.setBotmain(mock.Mock()))
