...
class FakeBuildRootTestCase(unittest.TestCase):
  """Sets up a fake build root (e.g., "<TEMP>/b/build/subordinate/buildername/build".
  ...
  @classmethod
  def setUpClass(cls):
    # Create a simulated buildsubordinate root.
    ...
    cls._builder_dir = os.path.join(
        cls._build_dir, 'subordinate', 'buildername', 'build')
