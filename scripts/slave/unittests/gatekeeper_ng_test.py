...
class GatekeeperTest(unittest.TestCase):
  def setUp(self):
    ...
    patcher = mock.patch('subordinate.build_scan._get_from_milo')
    ...
    for main in mains:
      ...
      for builder in main.builders:
        ...
        for build in builder.builds:
          ...
          self.build_json[(main.name, builder.name, build.number)] = (
              build_json)

        main_json['builders'][builder.name] = builder_json
      self.main_json[main.name] = main_json
  ...
  def create_generic_build_tree(self, main_title, main_name):
    build = GatekeeperTest.create_generic_build(1, ['a_committer@chromium.org'])

    builder = Builder('mybuilder', [build])

    url = self.main_url_root + main_name

    return Main(main_name, main_title, url, [builder])
  ...
  def _get_milo_handler(self, endpoint, data, milo_creds=None):
    ...
    if endpoint == 'milo.Buildbot/GetCompressedMainJSON':
      out = json.dumps(self.main_json[data['name']])
    ...
    if endpoint == 'milo.Buildbot/GetBuildbotBuildJSON':
      ...
      out = json.dumps(self.build_json[(main, builder, build_num)])
  ...
  def testFailedBuildDetected(self):
    ...
    self.argv.extend([m.url for m in self.mains])
  ...
  def testGlobbedExcludedBuildersDontCloseTree(self):
    ...
    glob = '%s*' % (self.mains[0].builders[0].name[0], )
    self.mains[0].builders[0].builds[0].steps[1].results = [2, None]
    self.add_gatekeeper_section(self.mains[0].url,
                                self.mains[0].builders[0].name,
                                {'closing_optional': ['step1'],
