...
"""A tool to list builder subordinates and trigger dependencies.

  Buildsystem capacity simulations require knowledge of builder->subordinate mappings
  ...
  Example usage: `scripts/tools/runit.py python scripts/tools/build_topology.py
                      mains/main.tryserver.chromium.linux`
"""
...
USAGE = '%s [options] main_dir' % sys.argv[0]


def get_topology_info(main_dir):
  config = main_cfg_utils.LoadConfig(main_dir)['BuildmainConfig']
  ...
  for b in config['builders']:
    # Add the subordinates to the builder entry.
    builders[b['name']]['subordinates'] = b['subordinatenames']
...
def main():
  ...
  if not args:
    option_parser.error('must specify a main directory!')
    return 1
  if len(args) > 1:
    option_parser.error('must specify only one main directory!')
